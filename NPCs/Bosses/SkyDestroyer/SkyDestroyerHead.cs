...
namespace TeaNPCMartianAddon.NPCs.Bosses.SkyDestroyer
{
    ...
    public class SkyDestroyerHead : SkyDestroyerSegment
    {
        ...
        public ModulePhase[] Modules { get; private set; } = new ModulePhase[]
        {
            ...
            new ModulePhase(3,new int[]{ SpaceWarp,WormBarrage, Plasmerizer, PlasmaWarpBlast, WarpMove, FireballBarrage,WormBarrage,PlasmaWarpBlast,WarpMove },0.45f,"Mods.TeaNPCMartianAddon.NPCChat.SDEX3"),
            new ModulePhase(4,new int[]{ LightningStormEx,Plasmerizer,SpaceWarp, AntimatterBomb, PlasmaWarpBlast },0,"Mods.TeaNPCMartianAddon.NPCChat.SDEX4"),
            new ModulePhase(5,new int[]{ ResetStates,DeathAnimation1 },0,"Mods.TeaNPCMartianAddon.NPCChat.SDEX5")
        };
        ...
        public override void AI()
        {
            ...
            if (player.dead && !playerDead)
            {
                SDMessage(Language.GetTextValue("Mods.TeaNPCMartianAddon.NPCChat.SD1"));
                playerDead = true;
            }
            #region Loomup
            if (base.NPC.alpha != 0)
            ...
            else if (NPC.ai[1] == -1)
            {
                if (NPC.ai[2] == 0)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        WarpMark = NPC.NewProjectile(warpTarget, Vector2.Zero, ModContent.ProjectileType<SkyShowupWarpMark>(),
                        NPC.damage/3, 0f, Main.myPlayer, warpStart.X, warpStart.Y);
                        NPC.netUpdate = true;
                    }
                    WarpState = 1;
                ...
                else
                {
                    if (Util.CheckProjAlive<SkyShowupWarpMark>(WarpMark))
                    {
                        ...
                        {
                            NPC.velocity = (mark.ProjAIToVector() - NPC.Center).SafeNormalize(Vector2.Zero) * maxSpeed / 2;
                        }
                    }
                    else
                    {
            ...
            if (NPC.ai[1] >= 0 && extraAI[0] == 1)
            {
                ...
                if (WarpState == 1)
                {
                    if (Util.CheckProjAlive<SkyWarpMark>(WarpMark))
                    {
                        ...
                        {
                            NPC.velocity = (mark.ProjAIToVector() - NPC.Center).SafeNormalize(Vector2.Zero) * maxSpeed;
                        }
                    }
                    else
                    {
                        SwitchTo(HandleWarpIssues);
                    }
                }
            }
            else if (NPC.ai[1] == LightningStorm)
            {
                ...
                if (NPC.ai[2] == 120 || NPC.ai[2] == 560 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    NPC.NewProjectile(start - Vector2.UnitX * 450, Vector2.Zero, ModContent.ProjectileType<SkyLightningOrb>(), NPC.damage/3,
            ...
            else if (NPC.ai[1] == HandleWarpIssues)
            {
                HandleWarpIssuesAI(player, maxSpeed, turnAcc, ramAcc);
            }
        ...
        void HandleWarpIssuesAI(Player player, float maxSpeed, float turnAcc, float ramAcc)
        {
            if (NPC.ai[2] == 0)
            {
                NPC.ai[2]++;
                ...
                Util.KillAll<SkyUFO>(true, 1);
                Util.KillAll<SkyPlasmaBallChainHead>(true);
                WarpMark = NPC.NewProjectile(player.Center + Main.rand.NextVector2Unit() * 200, NPC.Center + NPC.velocity * 20,  ModContent.ProjectileType<SkyWarpMarkEx>(),
            ...
            if (WarpState == 1)
            {
                if (Util.CheckProjAlive<SkyWarpMarkEx>(WarpMark))
                {
                    ...
                    {
                        NPC.velocity = (mark.velocity - NPC.Center).SafeNormalize(Vector2.Zero) * maxSpeed;
                    }
                }
                else
                {
        ...
        #region CommonHelper
        public int LocalCloestSegment => localClosestSegment;
                ...
                else
                {
                    ...
                    if (NPC.ai[2] % 10 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        NPC.NewProjectile(NPC.Center, Vector2.UnitY * direction * 8f, ModContent.ProjectileType<SkyFireballLauncher>(),
                            NPC.damage/4, 0f, Main.myPlayer, NPC.target, mode);
                    }
                }
            }
                ...
                if (NPC.ai[2] == 200 && CurrentModule.ID > 0 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = -5; i <= 5; i++)
                    {
                        ...
                        NPC.NewProjectile(pos, Vector2.UnitX * (-sign) * 6, ModContent.ProjectileType<SkyFireballLauncher>(),
                            NPC.damage / 4, 0f, Main.myPlayer, NPC.target, 2);
                        sign = -sign;
                    }
                }
        ...
        void PlasmerizerAI(Player player,float maxSpeed,float turnAcc,float ramAcc)
        {
            ...
            if (NPC.ai[2] % 45 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                NPC.NewProjectile(player.Center + offset, velo, ModContent.ProjectileType<SkyPlasmaBallChainHead>(), NPC.damage/3, 0f, Main.myPlayer, -1, 0);
            }
            if (NPC.ai[2] % 180 == 135)
            {
            ...
            if (NPC.ai[2] % 180 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                NPC.NewProjectile(NPC.Center+NPC.rotation.ToRotationVector2()*NPC.height, velo, ModContent.ProjectileType<SkyPlasmerizerRay>(), NPC.damage/3, 0f, Main.myPlayer);
            }
            if (NPC.ai[2] % 180 >= 80)
            {
        ...
        void AntimatterBombAI(Player player, float maxSpeed, float turnAcc, float ramAcc)
        {
            ...
            else if (NPC.ai[3] == 2)
            {
                if (NPC.ai[2] == 10 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    var start = NPC.Center + NPC.velocity * 20;
                    WarpMark = NPC.NewProjectile(warpTarget, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                        NPC.damage * 2 / 7, 0f, Main.myPlayer, start.X, start.Y);
                    WarpState = 1;
                    NPC.netUpdate = true;
                }
                if (NPC.ai[2] >= 10)
            ...
            if (NPC.ai[3] == 1 || NPC.ai[3] == 3)
            {
                if (NPC.ai[2] % 8 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    NPC.NewProjectile(NPC.Center, Vector2.UnitY * 2 * Math.Sign(player.Center.Y - NPC.Center.Y), ModContent.ProjectileType<SkyMatterMissile>(),
                        NPC.damage / 4, 0f, Main.myPlayer, NPC.whoAmI, 1);
                }
            }
        }
        #region WarpUtils
        ...
        void WarpMoveAI(Player player, float maxSpeed, float turnAcc, float ramAcc)
        {
            ...
            if (WarpState == 0)
            {
                if (NPC.ai[3] < times && NPC.ai[2] % 360 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    NPC.localAI[0] = NPC.NewProjectile(player.Center + Main.rand.NextVector2Unit() * 1200, NPC.Center + NPC.velocity * 20,  ModContent.ProjectileType<SkyWarpMarkEx>(),
                ...
                if (NPC.ai[2] % 360 == 60 && NPC.ai[3] < times)
                {
                    if (Util.CheckProjAlive<SkyWarpMarkEx>((int)NPC.localAI[0]) && NPC.localAI[0] != WarpMark)
                    {
                        ...
                        mark.Center = player.Center + Main.rand.NextVector2Unit() * length;
                    }
                }
                if (NPC.ai[2] % 360 <= 180  && NPC.ai[3] < times)
                {
                    ...
                    {
                        Projectile mark = Main.projectile[(int)NPC.localAI[0]];
                        mark.velocity = NPC.Center + NPC.velocity * 20;
                        mark.ai[0] = 11;
                        mark.netUpdate = true;
                ...
                NPC.WormMovementEx(player.Center, maxSpeed * 1.8f, turnAcc, ramAcc, angleLimit: MathHelper.PiOver2, distLimit: 1800,
                    radiusSpeed: 0.15f);
                    ...
                    if (NPC.ai[2] % 360 >= 240)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            NPC.NewProjectile(mark.Center,
                                ...
                                NPC.damage / 4, 0f, Main.myPlayer, SkyPortalSentry.PackAi0((0, 3)), 9);
                        ...
                        mark.localAI[1]++;
                        NPC.ai[3]++;
                        NPC.velocity = (player.Center + player.velocity * 20 - NPC.Center).SafeNormalize(Vector2.UnitX) * maxSpeed;
                    }
                }
            }
        ...
        void PlasmaWarpBlastAI(Player player, float maxSpeed, float turnAcc,float ramAcc)
        {
            if (NPC.ai[2] == 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    var start0 = NPC.Center + NPC.velocity * 30;
                    WarpMark = NPC.NewProjectile(visibleStart, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                    NPC.damage/3, 0f, Main.myPlayer, start0.X, start0.Y);
                    Main.projectile[WarpMark].localAI[1] = -1;
                    int portal = NPC.NewProjectile(visibleStart2, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                    NPC.damage/3, 0f, Main.myPlayer, visibleEnd.X, visibleEnd.Y);
                    var warpTarget = NPC.Center + NPC.DirectionTo(centerVector) * 1800;
                    int portal2 = NPC.NewProjectile(warpTarget, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                    NPC.damage/3, 0f, Main.myPlayer, visibleEnd2.X, visibleEnd2.Y);
            ...
            if (NPC.ai[2] % 45 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (NPC.ai[3] > 0)//one passed
                {
                    ...
                    NPC.NewProjectile(Vector2.Lerp(top, bottom, Main.rand.NextFloat()), velo, ModContent.ProjectileType<SkyPlasmaBallChainHead>(),
                        NPC.damage/3, 0f, Main.myPlayer, -1, 1);
                }
        ...
        void SpaceWarpAI(Player player,float maxSpeed,float turnAcc,float ramAcc)
        {
            ...
            if (NPC.ai[2] == 0)
            {
                ...
                WarpMark = NPC.NewProjectile(player.Center + Main.rand.NextVector2Unit() * 1200, NPC.Center + NPC.velocity * 20,  ModContent.ProjectileType<SkyWarpMarkEx>(),
                            0, 0f, Main.myPlayer, 11, 0);
                WarpState = 1;
                NPC.netUpdate = true; 
            }
            ...
            if (NPC.ai[2] <= 120)
            {
                ...
                if (NPC.ai[2] == 120 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    int lastId = -1;
                    var pos = centerVector + Vector2.UnitX * 4 * width * signX + Vector2.UnitY * height * sign;
                    ...
                    if (Util.CheckProjAlive<SkyWarpMarkEx>(WarpMark))
                    {
                        ...
                        if (NPC.ai[2] == 120)
                        {
                            ...
                            mark.localAI[1]++;
                            NPC.ai[3]++;
                            NPC.velocity = -Vector2.UnitY * sign * maxSpeed;
                        }
                    }
                    pos = centerVector + Vector2.UnitX * 4 * width * signX - Vector2.UnitY * height * sign;
                    ...
                    for (int k = -4; k <= 3; k++)
                    {
                        ...
                        if (lastId == -1)
                        {
                            ...
                            var start = NPC.NewProjectileDirect(begin2, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                            0, 0f, Main.myPlayer, pos.X, pos.Y);
                            start.localAI[0] = -1;
                            start.localAI[1] = -1;
#if !DEBUG
                        ...
                        proj = NPC.NewProjectileDirect(end, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                            0, 0f, Main.myPlayer, begin.X, begin.Y);
                        proj.localAI[0] = -1;
#if !DEBUG
                        proj.netUpdate = true;
#endif
                    ...
                    proj = NPC.NewProjectileDirect(warpTarget, Vector2.Zero, ModContent.ProjectileType<SkyWarpMark>(),
                            0, 0f, Main.myPlayer, start2.X, start2.Y);
                    proj.localAI[0] = -1;
                    Main.projectile[lastId].localAI[0] = proj.whoAmI;
                    WarpState = 1;
                    NPC.ai[3]++;
            ...
            else
            {
                if (WarpState == 0)
                {
                    ...
                    IdleMove(player, maxSpeed*1.8f, turnAcc, ramAcc);
                    if (Util.CheckProjAlive<SkyWarpMark>(WarpMark))
                    {
                        if (Main.projectile[WarpMark].localAI[0] != -1)
                        {
                            if (!Util.CheckProjAlive<SkyWarpMark>((int)Main.projectile[WarpMark].localAI[0]))
                            {
                                System.Diagnostics.Debugger.Break();
                            }
                            WarpMark = (int)Main.projectile[WarpMark].localAI[0];
                ...
                if (WarpState == 1)
                {
                    if (Util.CheckProjAlive<SkyWarpMark>(WarpMark))
                    {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient && mark.localAI[1] != 0)
                            {
                                ...
                                foreach (Vector2 pos in list)
                                {
                                    ...
                                    NPC.NewProjectile(pos, velo, ModContent.ProjectileType<SkyPlasmaRayLauncher>(),
                                        NPC.damage/3, 0f, Main.myPlayer, velo.ToRotation(), 3600);
                                }
                            }
                        ...
                        {
                            NPC.velocity = (mark.ProjAIToVector() - NPC.Center).SafeNormalize(Vector2.Zero) * maxSpeed * 3 / 5;
                        }
                    }
                    else
                    {
        ...
        {
            int times = 2;
            if (NPC.ai[2] % 75 == 0&&NPC.ai[2]%360!=180&&Main.netMode!=NetmodeID.MultiplayerClient)
            {
                var unit = NPC.localAI[1].ToRotationVector2();
                for (int i = 0; i < 5; i++)
                {
                    Vector2 offset = Vector2.UnitY * Main.rand.Next(-600, 600);
                    NPC.NewProjectile(player.Center + offset - unit * 1050, player.Center + offset + unit * 1350,
                        ModContent.ProjectileType<SkyAimLineEx>(), NPC.damage / 5, 0f, Main.myPlayer
                        , ModContent.ProjectileType<SkyPlasmaAcclerator>(), 4.5f);
                }
                NPC.NewProjectile(player.Center - unit * 1050, player.Center + unit * 1350,
                        ModContent.ProjectileType<SkyAimLineEx>(), NPC.damage / 5, 0f, Main.myPlayer
                        , ModContent.ProjectileType<SkyPlasmaAcclerator>(), 4.5f);
                unit = unit.RotatedBy(MathHelper.PiOver2 * (Main.rand.NextBool() ? -1 : 1));
                NPC.NewProjectile(player.Center - unit * 1050, player.Center + unit * 1050,
                        ModContent.ProjectileType<SkyAimLineEx>(), NPC.damage / 5, 0f, Main.myPlayer
                        , ModContent.ProjectileType<SkyPlasmaAcclerator>(), 4.5f);
            }
            if (WarpState == 0)
            {
                ...
                {
                    NPC.localAI[1] = (Main.rand.NextBool() ? 0 : 2) * MathHelper.PiOver2;
        ...
        void WormBarrageAI(Player player, float maxSpeed, float turnAcc, float ramAcc)
        {
            ...
            if (WarpState == 0)
            {
                ...
                {
                    NPC.localAI[0] = NPC.NewProjectile(player.Center + Main.rand.NextVector2Unit() * 1200, NPC.Center + NPC.velocity * 20, ModContent.ProjectileType<SkyWarpMarkEx>(),
                        0, 0f, Main.myPlayer, 0);
                }
                NPC.ai[2]++;
                if (NPC.ai[2] % 360 == 75 && NPC.ai[3] < times)
                {
                    if (Util.CheckProjAlive<SkyWarpMarkEx>((int)NPC.localAI[0]) && NPC.localAI[0] != WarpMark)
                    {
                        Projectile mark = Main.projectile[(int)NPC.localAI[0]];
                ...
                if (NPC.ai[3] >= times)
                {
                    if (NPC.ai[2] % 360 >= 120 && NPC.ai[2] % 360 < 280)
                    {
                        CurrentModule.SwitchToNext(this);
                        NPC.velocity = NPC.velocity.SafeNormalize(Vector2.UnitY) * maxSpeed;
                    }
                }
                if ((NPC.ai[2] % 360 < 280 && NPC.ai[2] % 360 >= 45) || times == 0)
                    NPC.WormMovementEx(player.Center, maxSpeed * 1.8f, turnAcc, ramAcc, angleLimit: MathHelper.PiOver4);
            }
            else if (WarpState == 1)
            {
                if (Util.CheckProjAlive<SkyWarpMarkEx>(WarpMark))
            ...
            else if (WarpState == 2)
            {
                ...
                if (Util.CheckProjAlive<SkyWarpMarkEx>(WarpMark))
                {
                    ...
                    if (NPC.ai[2] % 360 <= 280)
                    {
                        if (NPC.ai[2] % 75 == 0)
                        {
                            var offset = Main.rand.NextVector2CircularEdge(600, 400);
                            NPC.NewProjectile(player.Center + offset, offset.SafeNormalize(Vector2.Zero),
                                ModContent.ProjectileType<SkyWarpWorm>(), NPC.damage / 2, 0f, Main.myPlayer, 180, 2);
                        }
                        if (NPC.ai[2] % 360 == 200)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                NPC.NewProjectile(player.Center + player.velocity * 60, Vector2.Zero, ModContent.ProjectileType<SkyAim>(), 0,
                                    0f, Main.myPlayer, -1, 40);
                            }
                            mark.Center = player.Center + player.velocity * 60;
                        }
                        ...
                        if (NPC.ai[2] % 360 == 240)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                Projectile proj = NPC.NewProjectileDirect(mark.Center,
                                    Vector2.Zero
                                    , ModContent.ProjectileType<SkyAimLine>(),
                            ...
                            mark.ai[0] = 11;
                            mark.netUpdate = true;
                            mark.GetModProjectile<SkyWarpMarkEx>().ResetExtraAI(1);
                        }
                    }
                    else
                    {
                ...
                if (Util.CheckProjAlive<SkyWarpMarkEx>(WarpMark))
                {
                    ...
                    if (!IsWarping(2))
                    {
                        ...
                        if (NPC.ai[2] <= 180 && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            if (NPC.ai[2] % 20 == 0)
                            {
                                var offset = Main.rand.NextVector2Circular(1600, 1200);
                                NPC.NewProjectile(player.Center + offset, offset.SafeNormalize(Vector2.Zero), ModContent.ProjectileType<SkyWarpWorm>(),
                                    NPC.damage / 2, 0f, Main.myPlayer, 0, 3);
                            }
                        }
                        NPC.ai[2]++;
                        NPC.velocity *= 0.95f;
                            ...
                            if (NPC.ai[2] >= 660)
                            {
                                for (int i = 0; i < Main.maxGore; i++)
                                {
                                        ...
                                        || Main.gore[i].type == Mod.GetGoreType(SkyWarpWorm.TailGore)))
                                    {
                                        ...
                                        Main.gore[i].velocity += (NPC.Center - Main.gore[i].position).SafeNormalize(Vector2.Zero) * 0.8f;
                                    }
                                }
                            }
                        }
        ...
        public override bool CheckDead()
        {
            ...
            {
                NPC.life = 1;
                SwitchTo(DeathAnimation0);
                _currModuleIndex = 3;
                SDMessage(CurrentModule.EnterQuoteID);
                int i = NPC.whoAmI;
                int counter = 0;
                while (i != -1)
                {
        ...
        public float GetBodyMaxSpeed()
        {
            ...
            if (Main.expertMode)
                maxSpeed *= 1.125f;
            if (NPC.ai[1] == WarpMove||NPC.ai[1]==SpaceWarp ||NPC.ai[1]==HandleWarpIssues
                ||NPC.ai[1]==WormBarrage
                || NPC.ai[1]==LightningStormEx ||NPC.ai[1] == DeathAnimation0)
                maxSpeed *= 1.8f;
            return maxSpeed;
        }
